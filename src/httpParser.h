...
private:
    ...
    void parse_line(const std::string& line) {
        if (line.length() == 0) {
            auto it = std::find_if(std::begin(headers), std::end(headers), [](const HttpHeader& header){ return header.identifier == "Content-Length"; });
        ...
        if (parse_body && body.str().length() == body_size) complete = true;
    }
};
